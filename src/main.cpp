...
void integral(double x_from, double x_to, double y_from, double y_to, double precision, std::atomic<double> & res) {
    double local_res=0;
    for (double x = x_from; x <=x_to; x+=precision) {
        for (double y = y_from; y <=y_to; y+=precision) {
            local_res+=f(x,y);
        }
    }
    res = res + local_res;
...
double runner(double precission, int n_threads, double start_x,double end_x, double start_y, double end_y) {
    ...
    std::thread threds[n_threads];

    for (int i = 0; i < n_threads; ++i) {
        threds[i] = std::thread(integral, start_x+step*i, start_x+step*(i+1) - (i==n_threads-1?0:precission), start_y, end_y, precission, std::ref(res));
    }
